...
streamChartNew = do
  ...
  da `on` draw $ do
    (es, styleF) <- liftIO $ readIORef defaultParamsRef
    streamChartRenderBlocksCairo styleF es
...
redrawStreamChart (StreamChart drawingArea scRef) = do
  ...
  drawingArea `on` draw $ streamChartRenderBlocksCairo styleF es
...
invalidateStreamChart (StreamChart da _) =
  widgetQueueDraw da
